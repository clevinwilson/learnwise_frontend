       ...
       <div>
            <div className="flex flex-col p-2 py-6 mb-6 m-h-screen items-center">
                <div className="bg-white items-center justify-between w-full flex rounded-full max-w-2xl drop-shadow-2xl p-2 mb-5 sticky" style={{ top: 5 }}>
                    <input ref={inputRef} value={query} onChange={(e) => { setQuery(e.target.value)}} className="font-bold uppercase rounded-full w-full py-4 pl-4 text-gray-700 bg-gray-100 leading-tight focus:outline-none focus:shadow-outline lg:text-sm text-xs" type="text" placeholder="Search" />
            ...
            {course?
                <div className='mx-3 lg:mx-20 mb-8'>
                    <h3 className='ml-5 text-2xl md:text-3xl'>{course ? course.length : " "} results for “{query}”</h3>
                </div>
                :""}

                {course && course.map((obj)=>{
                    return(
                        <div className='mx-3 lg:mx-20 mb-10'>
                            <div className="flex justify-center mt-4 sm:mx-10 m-3">
                                <div className="flex p-4 w-full max-w-screen-lg hover:bg-violet-50 flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row  dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                                    <img className="rounded-md mt-4 sm:mt-0 w-56 h-32 object-cover" src="http://localhost:3000/images/course/thumbnail/1679497306716-course1.jpg" alt />
